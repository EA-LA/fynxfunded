...
import { publicOrderReference } from "@/lib/publicReferences";
...
export default function Billing() {
  ...
  const resume = () => setHoldOpen(true);
  return <div className="space-y-6">
    <PaymentHold open={holdOpen} onOpenChange={setHoldOpen} />
    <div><p className="text-xs font-semibold uppercase tracking-[.18em] text-muted-foreground">Orders & payments</p><h1 className="mt-2 text-3xl font-bold tracking-tight">Billing</h1><p className="mt-2 text-sm text-muted-foreground">Review purchases and access order records. Payments are on hold while we finish the final step.</p></div>
    ...
    <div className="overflow-hidden rounded-lg border border-border bg-card"><div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div className="relative max-w-sm flex-1"><Search size={15} className="absolute left-3 top-2.5 text-muted-foreground"/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search orders..." className="h-9 w-full rounded-md border border-border bg-secondary/30 pl-9 pr-3 text-sm outline-none"/></div><div className="flex gap-1">{["all","pending","paid","failed","refunded"].map(x=><button key={x} onClick={()=>setStatus(x)} className={`rounded-md px-3 py-2 text-xs capitalize ${status===x?"bg-primary text-primary-foreground":"bg-secondary text-muted-foreground"}`}>{x}</button>)}</div></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[900px] text-sm"><thead className="bg-secondary/30 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-4 py-3">Challenge / receipt</th><th className="px-4 py-3">Date</th><th className="px-4 py-3">Amount</th><th className="px-4 py-3">Payment</th><th className="px-4 py-3">Status</th><th className="px-4 py-3">Action</th></tr></thead><tbody>{visible.map(o=><tr key={o.id} className="border-t border-border"><td className="px-4 py-4"><p className="font-medium">{o.challenge || `${money(o.accountSize)} challenge`}</p><p className="mt-1 font-mono text-xs text-muted-foreground">Receipt {publicOrderReference(o)}</p></td><td className="px-4 py-4">{date(o.paidAt || o.createdAt)}</td><td className="px-4 py-4 font-semibold">{money(o.amount,o.currency)}</td><td className="px-4 py-4 capitalize">{o.paymentMethod || "card"}</td><td className="px-4 py-4"><Badge value={o.status}/></td><td className="px-4 py-4">{o.status === "pending" ? <button onClick={()=>resume()} className="rounded-md bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground">Pay now</button> : <button onClick={()=>void downloadReceipt({...o,orderId:o.id})} className="rounded-md border border-border px-3 py-2 text-xs">Download PDF</button>}</td></tr>)}</tbody></table>{!visible.length&&<div className="py-16 text-center"><ReceiptText className="mx-auto mb-3 text-muted-foreground"/><p className="text-sm font-medium">No orders found</p><p className="mt-1 text-xs text-muted-foreground">New and completed checkout orders will appear here.</p></div>}</div>
    </div>
  </div>;
}
